...
def main() -> None:
    ...
    for mod, margs in (("universe", ["--launch-start", since, "--launch-end", until, "--append"]),
        ...
        if not ok:
            ...
            # ПЕЧАТАЕМ ТОЖЕ В stdout (правка 12.08). Причина отказа уходила ТОЛЬКО в
            # Telegram, а крон-скрипт пишет в лог именно stdout — поэтому лог провала
            # выглядел пустым, и три недели было не понять, почему автообновление не
            # работает. Настоящая причина оказалась прозаичной: предыдущий контейнер
            # discovery ещё жив и держит блокировку DuckDB.
            print(f"ПРОВАЛ на шаге {mod}: {err}", flush=True)
