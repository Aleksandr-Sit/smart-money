...
def _рост_токенов() -> list[tuple[float, str]]:
...
def победители(мин_рост: float = 10.0) -> list[tuple[float, str]]:
    return [(r, m) for r, m in _рост_токенов() if r >= мин_рост]


def неудачники(макс_рост: float = 2.0, сколько: int = 40) -> list[tuple[float, str]]:
    """КОНТРОЛЬНАЯ ГРУППА: токены, которые НЕ выросли.

    Без неё список кандидатов бессмыслен. Кошелёк, встретившийся в пяти победителях,
    выглядит находкой — пока не выяснится, что он же сидит в пятидесяти неудачниках.
    Это просто высокочастотный бот, покупающий всё подряд, и его «попадания» —
    арифметика больших чисел, а не отбор.

    Берём из СЕРЕДИНЫ списка (не самый хвост): токены, вовсе не сдвинувшиеся с места,
    часто не имеют покупателей вообще, и контроль выродился бы в пустоту.
    """
    все = [(r, m) for r, m in _рост_токенов() if r < макс_рост]
    if not все:
        return []
    шаг = max(1, len(все) // сколько)
    return все[::шаг][:сколько]
...
def собрать(мин_рост: float = 10.0, токенов: int = 40, ранних: int = 25,
            за_прогон: int | None = None, контроль: int = 0) -> dict:
    """`за_прогон` — сколько НОВЫХ токенов разбирать за запуск (остальные из кэша).
    `контроль` — сколько НЕвыросших токенов разобрать для знаменателя."""
    поб = победители(мин_рост)[:токенов]
    победные = {m for _, m in поб}
    if контроль:
        # КОНТРОЛЬ — ВСЁ РАЗОБРАННОЕ, ЧТО НЕ ПОБЕДИТЕЛЬ, а не свежая выборка из
        # `неудачники()`. Первая версия звала её при каждом запуске, а список токенов
        # растёт с приходом данных — выбор каждый раз получался другой, и из сорока
        # разобранных контрольных в знаменатель попадали два. «Подъём» выходил
        # стомиллионным, то есть бессмысленным.
        уже = [(0.0, m) for m in _кэш() if m not in победные]
        нужно = максимум = контроль - len(уже)
        поб = поб + уже + (неудачники(сколько=нужно) if максимум > 0 else [])
    if not поб:
    ...
    счёт_к: Counter = Counter()
    места: dict[str, list[int]] = defaultdict(list)
    разобрано = разобрано_к = 0
    for рост, m in поб:
        кош = кэш.get(m)
        if not кош:
            continue
        if m not in победные:
            разобрано_к += 1
            for w in кош:
                счёт_к[w] += 1
            continue
    ...
    for w, c in счёт.items():
        if c < 2:
            continue
        неудач = счёт_к.get(w, 0)
        # ПОДЪЁМ СО СГЛАЖИВАНИЕМ (правка 12.08). Прямое отношение долей даёт
        # бесконечность любому кошельку с нулём неудачников — и двойка попаданий
        # вытесняет девятку, потому что обе делятся на ноль. Сглаживание Лапласа
        # возвращает смысл: при равном знаменателе выигрывает тот, у кого больше
        # попаданий, а один случайный хит не поднимается наверх.
        # 1.0 = кошелёк покупает всё подряд, совпадения с победителями случайны.
        подъём = None
        if разобрано_к:
            подъём = (((c + 1) / (разобрано + 2))
                      / ((неудач + 1) / (разобрано_к + 2)))
        кандидаты.append({"кошелёк": w, "победителей": c, "доля": c / max(разобрано, 1),
                          "неудачников": неудач, "подъём": подъём,
    ...
    d = {"мало_данных": False, "победителей": len(победные), "разобрано": разобрано,
         "осталось_разобрать": осталось, "контроль_разобрано": разобрано_к,
...
def отчёт(d: dict, показать: int = 15) -> str:
    ...
    if d.get("контроль_разобрано"):
        строки.append(f"контрольная группа (НЕвыросшие токены): {d['контроль_разобрано']} шт")
        строки.append("")
    if d["новых"]:
        есть_подъём = any(k.get("подъём") is not None for k in d["новых"])
        строки.append(f'{"кошелёк":>10s} {"победителей":>12s} {"неудачников":>12s} '
                      f'{"подъём":>7s} {"место":>6s}')
        показ = sorted(d["новых"], key=lambda k: -(k.get("подъём") or 0))             if есть_подъём else d["новых"]
        for k in показ[:показать]:
            п = k.get("подъём")
            строки.append(f'{k["кошелёк"][:8]:>10s} {k["победителей"]:12d} '
                          f'{k.get("неудачников", 0):12d} '
                          f'{("н/д" if п is None else f"{п:.1f}x"):>7s} '
                          f'{k["медиана_места"]:6d}')
        if not есть_подъём:
            строки.append("")
            строки.append("ПОДЪЁМА НЕТ — контрольная группа не разобрана. Без неё частый бот, "
                          "покупающий всё подряд, неотличим от находки: попадания в победителей "
                          "у него просто арифметика больших чисел. Прогнать с --контроль.")
        строки.append("")
...
def main() -> None:
    ...
    ap.add_argument("--контроль", type=int, default=0,
                    help="сколько НЕвыросших токенов разобрать для знаменателя")
    ap.add_argument("--telegram", action="store_true")
    a = ap.parse_args()
    d = собрать(getattr(a, "рост"), getattr(a, "токенов"), getattr(a, "ранних"),
                getattr(a, "за_прогон"), getattr(a, "контроль"))
    txt = отчёт(d)
    print(txt)
    if a.telegram:
