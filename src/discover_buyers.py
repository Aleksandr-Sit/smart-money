...
def собрать(мин_рост: float = 10.0, токенов: int = 40, ранних: int = 25,
    ...
    if контроль:
        поб = поб + неудачники(сколько=контроль)
    if not поб:
    ...
    победные = {m for r, m in поб if r >= мин_рост}
    for рост, m in поб:
        кош = кэш.get(m)
        if not кош:
            continue
    ...
    for w, c in счёт.items():
        ...
        # ПОДЪЁМ: во сколько раз чаще кошелёк встречается у победителей, чем у
        # неудачников. 1.0 = покупает всё подряд, и совпадения случайны.
        подъём = None
        if разобрано_к:
            подъём = (c / разобрано) / max(неудач / разобрано_к, 1e-9)
