...
async def run(max_mc: float, seconds: int | None) -> None:
    ...
    повтор: dict[str, str] = {}        # токен → причина выхода, продажу которого надо повторить
    ...
    async def _emit_exit(token: str, exit_price: float | None, reason: str) -> None:
        ...
        if not sold:
            ...
            # ПОВТОР НА БЛИЖАЙШЕМ ТИКЕ (замер 10.08). Прежде сорванная продажа просто
            # оставляла позицию открытой: актор уже помечен вышедшим, поэтому on_sell
            # больше не сработает, а ценовые правила ждут своего часа — и позиция
            # доживала до таймаута. Медианная задержка до итоговой продажи выходила
            # 259с, и цена за это время уходила на −15.40%.
            # Что дал бы повтор (45 цепочек, цены из траекторий):
            #     через 15с  медиана −4.29%  сумма +$17.17   <- выбрано, совпадает с тиком
            #     через 30с  медиана +0.95%  сумма  +$4.59
            #     через 60с  медиана −2.38%  сумма  −$6.91
            #     фактически медиана −15.40%  сумма  +$0.72
            повтор[token] = reason
            return
        ...
        # ПОЛ −100%: купив токен на клип, нельзя потерять больше клипа. Значение ниже
        # означает не убыток, а сломанный ЗАМЕР, и доверять ему нельзя ни в отчёте, ни
        # в дневном стопе. Случай 10.08: параллельная покупка попала в замер выручки и
        # дала «realized −200%» при цене выхода 0.96x.
        if факт is not None and факт < -1.0:
            print(f"[EXIT ЗАМЕР СЛОМАН] {token[:12]} факт {факт:+.1%} невозможен — "
                  f"беру модельный {r:+.1%}")
            await loop.run_in_executor(
                None, delivery.send_alert,
                f"ЗАМЕР ВЫРУЧКИ СЛОМАН {token[:12]}: получилось {факт:+.0%}, что "
                f"невозможно для спот-сделки. Итог взят по модели ({r:+.0%}), "
                f"дневной счётчик не испорчен.")
            факт = None
        ...
        # МУЛЬТИПЛИКАТИВНЫЙ разрыв — доля модельного результата, дошедшая до кошелька.
        # Вычитание доходностей врёт тем сильнее, чем крупнее сделка: 13.08 модель дала
        # +359%, деньги +7%, и арифметическая разница вышла −352% — число, которое
        # нечего делать в сообщении о сделке на $10. Отношение (1+факт)/(1+модель) в том
        # же случае даёт 0.23, то есть «до нас дошла четверть», и это читается.
        # Замер по 176 живым сделкам: медиана 0.891, порог окупаемости 0.911.
        разрыв_к = ((1 + факт) / (1 + r)
                    if (факт is not None and r is not None and r > -0.999) else None)
        ...
        if _actor:
            try:
                delivery.log_actor_buy(_actor[0], wallet, token, ev.usd, ev.ts,
                                       ...
                                       price, (trade.get("curve_price_sol") or 0) * sol
                                       or None)
            except Exception:  # noqa: BLE001
    ...
    async def exit_tick(prices: dict) -> None:
        ...
        # СНАЧАЛА ПОВТОРЯЕМ СОРВАННЫЕ ПРОДАЖИ (замер 10.08). Решение выйти уже принято —
        # незачем ждать, пока сработает какое-нибудь другое правило. Раньше позиция после
        # сорванной продажи доживала до таймаута: медиана 259с, цена уходила на −15.40%.
        for token, причина in list(повтор.items()):
            if token not in pm.open_tokens():
                повтор.pop(token, None)
                continue
            цена_сейчас = prices.get(token) or tracker.last.get(token)
            print(f"[EXIT повтор] {token[:12]} ({причина})")
            await emit_exit(token, цена_сейчас, причина)
