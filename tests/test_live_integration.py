...
def test_инварианты_боевого_конфига_при_живых_деньгах():
    ...
    if not live:
        # ДВА ЗАКОННЫХ СОСТОЯНИЯ БЕЗ ЖИВЫХ ДЕНЕГ, и их нельзя путать:
        #   shadow  — бумажный эксперимент: денежные лимиты пишутся в журнал, но
        #             поток не режут, иначе замер собирал бы усечённую выборку;
        #   enforce — ВЕТКА ЗАКРЫТА (14.08): лимиты режут по-настоящему, чтобы
        #             остановка держалась, а монитор при этом продолжал вести
        #             actor_buys, price_history и signals. Сбор данных бесплатен
        #             и нужен как база сравнения для площадки с издержками 0.09%.
        # Требовать здесь только shadow значило бы, что закрыть ветку без правки
        # теста нельзя, — а именно это и понадобилось.
        assert strategy.RISK["RISK_MODE"] in ("shadow", "enforce")
        return
