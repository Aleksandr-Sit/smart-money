...
def test_подъём_отличает_отбор_от_высокой_частоты(кэшируемый, monkeypatch):
    """Кошелёк в пяти победителях выглядит находкой, пока не выяснится, что он же
    сидит в пятидесяти неудачниках. Подъём 1.0 = покупает всё подряд."""
    поб = [(30.0, "П1"), (25.0, "П2")]
    неуд = [(1.1, "Н1"), (1.0, "Н2")]
    пок = {"П1": ["ОТБОРЩИК", "ВЕЗДЕСУЩИЙ"], "П2": ["ОТБОРЩИК", "ВЕЗДЕСУЩИЙ"],
           "Н1": ["ВЕЗДЕСУЩИЙ"], "Н2": ["ВЕЗДЕСУЩИЙ"]}
    кэшируемый(поб, пок)
    monkeypatch.setattr(db, "неудачники", lambda макс_рост=2.0, сколько=40: неуд)
    d = db.собрать(мин_рост=20.0, контроль=2)
    по = {k["кошелёк"]: k for k in d["кандидаты"]}
    assert d["контроль_разобрано"] == 2
    # со сглаживанием подъём конечен, поэтому сравниваем порядок, а не порог
    assert по["ОТБОРЩИК"]["подъём"] > по["ВЕЗДЕСУЩИЙ"]["подъём"], "только у победителей"
    assert по["ВЕЗДЕСУЩИЙ"]["подъём"] < 1.5, "есть и там, и там — отбора нет"


def test_без_контроля_отчёт_предупреждает(сбор):
    сбор([(20.0, "A"), (15.0, "B")], {"A": ["W", "W2"], "B": ["W", "W2"]})
    т = db.отчёт(db.собрать())
    assert "ПОДЪЁМА НЕТ" in т and "--контроль" in т


def test_контрольные_токены_не_считаются_победителями(кэшируемый, monkeypatch):
    """Иначе знаменатель «доля победителей» раздуется контролем и всё занизится."""
    кэшируемый([(30.0, "П1")], {"П1": ["W", "W2"], "Н1": ["W"]})
    monkeypatch.setattr(db, "неудачники", lambda макс_рост=2.0, сколько=40: [(1.0, "Н1")])
    d = db.собрать(мин_рост=20.0, контроль=1)
    assert d["победителей"] == 1
    assert d["разобрано"] == 1


def test_контроль_берёт_всё_разобранное_а_не_свежую_выборку(кэшируемый, monkeypatch):
    """Первая версия звала неудачники() при каждом запуске, а список токенов растёт
    с приходом данных — выбор каждый раз получался другой. Из сорока разобранных
    контрольных в знаменатель попадали два, и подъём выходил стомиллионным."""
    кэшируемый([(30.0, "П1")], {"П1": ["W"], "СТАРЫЙ_КОНТРОЛЬ": ["W"]})
    db.собрать(мин_рост=20.0)                       # разобрали только победителя
    db._записать_кэш({**db._кэш(), "СТАРЫЙ_КОНТРОЛЬ": ["W"]})
    monkeypatch.setattr(db, "неудачники", lambda макс_рост=2.0, сколько=40: [])
    d = db.собрать(мин_рост=20.0, контроль=5)
    assert d["контроль_разобрано"] == 1, "уже разобранный неудачник обязан войти в знаменатель"


def test_подъём_сглажен_и_учитывает_число_попаданий(кэшируемый, monkeypatch):
    """Без сглаживания любой кошелёк с нулём неудачников получал бесконечность,
    и два попадания вытесняли девять — обе величины делились на ноль."""
    поб = [(30.0, f"П{i}") for i in range(10)]
    пок = {f"П{i}": ["ЧАСТЫЙ"] for i in range(10)}
    пок["П0"] = ["ЧАСТЫЙ", "РЕДКИЙ"]
    пок["П1"] = ["ЧАСТЫЙ", "РЕДКИЙ"]
    пок["Н1"] = ["ПОВСЮДУ"]
    кэшируемый(поб, пок)
    monkeypatch.setattr(db, "неудачники", lambda макс_рост=2.0, сколько=40: [(1.0, "Н1")])
    d = db.собрать(мин_рост=20.0, контроль=1)
    по = {k["кошелёк"]: k["подъём"] for k in d["кандидаты"]}
    assert d["контроль_разобрано"] == 1
    assert по["ЧАСТЫЙ"] > по["РЕДКИЙ"], "больше попаданий при том же нуле неудачников"
    assert all(v < 1e6 for v in по.values()), "бесконечностей быть не должно"
